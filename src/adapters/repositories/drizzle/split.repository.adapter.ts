...
export class DrizzleSplitRepositoryAdapter implements ISplitRepository {
	...
	async findByIdWithDetails(
	...
	): Promise<SplitWithDetailsDto | undefined> {
		...
		return {
			split: {
				id: split.split.id,
				userId: split.split.userId,
				title: split.split.title,
				description: split.split.description,
				isPublic: split.split.isPublic,
				isDefault: split.split.isDefault,
	...
	async findDaysBySplitId(splitId: string) {
	...
	async findWithFilters(
		...
		return results.map((result) => {
			...
			return {
				split: {
					id: result.split.id,
					userId: result.split.userId,
					title: result.split.title,
					description: result.split.description,
					isPublic: result.split.isPublic,
					isDefault: result.split.isDefault,
	...
	async createWithDays(input: CreateSplitDto): Promise<Split> {
		return await this.db.transaction(async (tx) => {
			...
			return this.toEntity(split);
		});
	}

	async update(id: string, input: UpdateSplitDto): Promise<Split> {
		const [updatedSplit] = await this.db
			...
			.where(eq(splits.id, id))
			.returning();
		return this.toEntity(updatedSplit);
	}
	...
	private toEntity(raw: typeof splits.$inferSelect): Split {
		return new Split(
			raw.id,
			raw.userId,
			raw.title,
